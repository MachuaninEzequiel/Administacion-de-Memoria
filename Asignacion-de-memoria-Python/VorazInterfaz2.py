    ...
    def get_allocation(self):
        ...
        for process in self.processes:
            if process.allocated:
                allocation.append(
                    f"Process {process.process_id}: Allocated from address {process.start_address} to {process.end_address}"
                )
        return allocation
...
class Application(tk.Tk):
    ...
    def add_process(self):
        ...
        process = Process(self.process_id, size, execution_time)
        self.process_id += 1
        if self.memory_simulator is None:
            memory_size = int(self.memory_size_entry.get())
            self.memory_simulator = MemorySimulator(memory_size)
        self.memory_simulator.add_process(process)
        self.processes_entry.insert(tk.END, f"{size}, {execution_time}\n")
    ...
    def parse_processes(self, processes_str):
        try:
            ...
            for process_data in processes_data:
                size, execution_time = map(int, process_data.split(","))
                process = Process(self.process_id, size, execution_time)
                self.process_id += 1
